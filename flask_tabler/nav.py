...
class TablerRenderer(Visitor):
    ...
    def visit_Navbar(self, node):
        ...
        collapse_button = title_content.add(
            button(_class="navbar-toggler", type="button")
        )
        ...
        title = title_content.add(
            a(
                node.title,
                _class="navbar-brand navbar-brand-autodark d-none-navbar-horizontal pe-0 pe-md-3",
                href=node.get_url() if hasattr(node, "get_url") else ".",
            )
        )
        nav = title_content.add(
            div(_class="collapse navbar-collapse", id="navbar-menu")
        )
        ...
        with nav:
            with div(
                _class="d-flex flex-column flex-md-row flex-fill align-items-stretch align-items-md-center"
            ):
    ...
    def visit_View(self, node):
        if self._in_dropdown:
            item = a(
                node.text, _class="dropdown-item", href=node.get_url(), title=node.text
            )
        else:
            item = li(_class=f'nav-item {"active" if node.active else ""}')
            item.add(
                a(node.text, _class="nav-link", href=node.get_url(), title=node.text)
            )
            ...
            with user:
                with a(
                    ...
                    with div(_class="d-none d-xl-block ps-2"):
                        div(username)
                        role = (
                            ", ".join([role.name for role in current_user.roles])
                            if len(current_user.roles) > 0
                            else "User"
                        )
                        div(role, _class="mt-1 small text-secondary")
                with div(
                    _class="dropdown-menu dropdown-menu-end dropdown-menu-arrow",
                    data_bs_popper="static",
                ) as user_list:
                    self._in_dropdown = True
                    for item in node.items:
